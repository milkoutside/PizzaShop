...
export class CartModal{
    ...
    refreshCart()
    {
        localStorage.setItem('r_fc','true')
        location.reload();
    }
    createProductsCard(cartList, amount)
    {
     ...
     <div class="cart-item">
            <div id="deleteAll-${cartList.productId}" class="cart-item-close">
            ...
            <div class="button-card-content-price-qtn">
                ...
                <div class="button-card-qty-block">
                    <div class="quantity-control">
                        <button id="deleteQty-${cartList.productId}" class="quantity-control-btn" type="button">
                        ...
                        <span>${amount}</span>
                        <button id="addQty-${cartList.productId}" class="quantity-control-btn" type="button">
...
`
            ...
            success: function (response) {
                ...
                for (let i = 0; i < cart.amount.length; i++)
                {
                    if(cart.cartProducts[i].productId === cart.amount[i][0]) {
                        ...
                        $(document).on("click", "#deleteAll-" + cart.cartProducts[i].productId, function () {
                            cc.deleteAllItems(cart.cartProducts[i].productId, cart.amount.length)
                            cardM.refreshCart()
                        });
                        $(document).on("click", "#deleteQty-" + cart.cartProducts[i].productId, function () {
                            cc.deleteFromCart(cart.cartProducts[i].productId)
                            cardM.refreshCart()
                        });
                        $(document).on("click", "#addQty-" + cart.cartProducts[i].productId, function () {
                            cc.addToCart(cart.cartProducts[i])
                            cardM.refreshCart()
                        });
                    }
                }
    ...
    export function addEventsCartModal()
    {
        let cartM = new CartModal();
